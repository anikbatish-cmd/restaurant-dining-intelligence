...
st.markdown(
    ...
    <div class="hero">
        ...
        <div class="muted">
            Competitive position, customer voice, marketing traction and consultant-grade
            recommendations built from observable public dining data.
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
if submitted:
    ...
    with st.status(
        "Building consultant-grade public intelligence...",
        expanded=True,
    ) as status:
        ...
        customer_voice = analyze_customer_voice(
            public_intel.get("reviews", [])
        )
        earned = analyze_earned_attention(
            public_intel.get("earned", [])
        )
        ...
        paid_signal = detect_paid_signal(
            public_intel.get("paid", []),
            restaurant,
        )
        ...
        cross_source_metrics = build_cross_source_metrics(
            source_summaries
        )
        ...
        workspace = consultant_workspace(
            insights,
            customer_voice,
            benchmark_target or {},
        )
        ...
        status.update(
            label="Dining Intelligence report ready",
            state="complete",
        )
    ...
    with coverage_col:
        ...
        st.metric("Public-data coverage", f"{coverage}%")

    if not district_summary:
        st.warning(
            "A high-confidence District snapshot was not available, so the report "
            "uses the strongest available dining source for directional analysis."
        )
    ...
    with tabs[0]:
        ...
        st.caption(
            "Directional public-market score — not revenue, footfall, profitability or ROAS."
        )
        ...
        with top4:
            ...
            st.metric(
                ...
                delta="rating/review volume vs median"
                if volume_index is not None
                else None,
            )
        ...
        if insights:
            ...
            st.info(
                "Public evidence is currently too thin for a strong diagnosis. "
                "The Evidence tab shows exactly what was and was not observable."
            )
        ...
        with x1:
            st.metric(
                "Observed rating spread",
                f"{cross_source_metrics['rating_spread']:.1f}"
        ...
        with x2:
            st.metric(
                ...
                fmt_money(cross_source_metrics.get("price_spread")),
            )
        with x3:
            st.metric(
    ...
    with tabs[1]:
        ...
        st.caption(
            "Cohort is selected from District using cuisine, price, location and "
            "occasion/positioning similarity. Match score is a comparability score, not performance."
        )
        ...
        if not comp_df.empty:
            st.dataframe(
                ...
                column_config={
                    ...
                    "Top Offer %": st.column_config.NumberColumn(
                        format="%d%%"
                    ),
                },
            )
            ...
            if len(chart_df) >= 2:
                chart_df["Ratings / Reviews"] = (
                    chart_df["Ratings / Reviews"].fillna(1).clip(lower=1)
                )
        ...
        with b1:
            st.metric(
                ...
                if competitive_metrics.get("cohort_rating_median") is not None
                else "—",
            )
        with b2:
            st.metric(
        ...
        with b3:
            st.metric(
                ...
                fmt_pct(competitive_metrics.get("reputation_percentile")),
            )
        with b4:
            st.metric(
                ...
                (
                    ...
                    if competitive_metrics.get("premium_justification_gap") is not None
                    else "—"
                ),
            )
        ...
        with st.expander("Why these competitors?"):
            for item in competitors:
                ...
                st.write(
                    f"Cuisine {parts['cuisine_similarity']:.0%} · "
                    f"Price {parts['price_similarity']:.0%} · "
                    f"Positioning {parts['positioning_similarity']:.0%}"
                )
                if item.get("url"):
    ...
    with tabs[2]:
        ...
        st.caption(
            "Directional topic analysis from search-visible public review snippets. "
            "This is not a complete review census."
        )
        ...
        with v3:
            strongest = (
                customer_voice["strengths"][0]["topic"]
        ...
        if topic_rows:
            ...
            st.bar_chart(
                topic_df.set_index("Topic")[["Mentions"]]
            )
        ...
        with voice_left:
            ...
            if customer_voice.get("strengths"):
                for item in customer_voice["strengths"]:
                    st.success(
                        f"**{item['topic']}** — {item['mentions']} observed mentions"
                    )
            else:
    ...
    with tabs[3]:
        ...
        st.caption(
            "Only observable public signals are shown. No spend, ROAS or booking impact is inferred."
        )
        ...
        with m4:
            ...
            st.metric(
                ...
                f"{lift:.1f}x" if lift is not None else "—",
            )
        with m5:
        ...
        with c_left:
            ...
            st.metric(
                "Creator/UGC items in sample",
                content_summary.get("creator_count", 0),
            )
            st.metric(
                "Owned items in sample",
                content_summary.get("owned_count", 0),
            )
            ...
            if content_items:
                with st.expander("Content evidence"):
                    for item in content_items[:15]:
                        st.markdown(
                            f"**{item['type']} · {item['theme']}**"
                        )
                        if item.get("engagement") is not None:
                            st.caption(
                                f"Visible engagement: {item['engagement']:,}"
                            )
        ...
        with d1:
            ...
            st.metric(
                "Share of observed cohort mentions",
                f"{share:.0%}" if share is not None else "—",
            )
        with d2:
            st.metric(
                "Target mentions",
                discovery.get("target_mentions", 0),
            )
        ...
        for row in discovery.get("queries", []):
            discovery_rows.append(
                {
                    "Search": row["query"],
                    restaurant: "Yes" if row["target_found"] else "No",
                    "Competitors surfaced": ", ".join(row["competitors_found"]) or "—",
                }
            )
    ...
    with tabs[4]:
        ...
        st.caption(
            "How the restaurant is publicly priced, promoted and positioned across observable sources."
        )
        ...
        if benchmark_target:
            ...
            with p1:
                st.metric(
                    "Primary rating",
                    f"{benchmark_target.get('rating'):.1f}"
                    if benchmark_target.get("rating")
                    else "—",
                )
            with p2:
                st.metric(
                    "Cost for two",
                    fmt_money(benchmark_target.get("cost_for_two")),
                )
            with p3:
                st.metric(
                    "Top visible offer",
                    benchmark_target["offers"][0]
            ...
            with p4:
                st.metric(
                    "Price percentile",
                    fmt_pct(competitive_metrics.get("price_percentile")),
                )
            ...
            source_rows.append(
                {
                    ...
                    "Top Offer": summary["offers"][0]
                    if summary.get("offers")
                    else None,
        ...
        positive_topics = [
            item["topic"]
            for item in customer_voice.get("strengths", [])
        ]
        concern_topics = [
            item["topic"]
            for item in customer_voice.get("concerns", [])
        ]

        if benchmark_target and benchmark_target.get("positioning_tags"):
            st.success(
                "Public promise: "
                + ", ".join(benchmark_target["positioning_tags"][:5])
            )
        if positive_topics:
            st.success(
                "Observable customer proof: "
                + ", ".join(positive_topics)
            )
        if concern_topics:
            st.warning(
                "Execution risks to validate: "
                + ", ".join(concern_topics)
            )
    ...
    with tabs[5]:
        ...
        st.caption(
            "A conversation-ready briefing for the restaurant owner. "
            "Every statement is designed to remain within what public evidence can support."
        )
    ...
    with tabs[6]:
        ...
        st.caption(
            "Use this tab when an owner challenges a claim. High-confidence values come from "
            "direct public pages; search snippets and web observations remain clearly labelled."
        )
        ...
        st.dataframe(
            pd.DataFrame(dining_metrics.get("direct_page_debug", [])),
            use_container_width=True,
            hide_index=True,
        )
    ...
    st.divider()
    st.caption(
        "Claim boundary: This report describes observable public-market signals. "
        "It does not infer footfall, bookings, revenue, profitability, paid spend, ROAS or causal business impact "
        "without internal restaurant data."
    )
